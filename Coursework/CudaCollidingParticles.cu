...
__device__
void CollideParticleWithCPUParticle(float baumgarte_factor, Particle& particle1, Particle& out_particle,
	float3 projpos, float3 projvel);
...
void CollideParticles(float baumgarte_factor, uint num_particles, Particle* particles, Particle* out_particles, uint* grid_cell_start, uint* grid_cell_end,
	uint numElems, float3* d_projpos, float3* d_projvel)
{
	...
	{
		CollideParticleWithCPUParticle(baumgarte_factor, out_p, out_p, d_projpos[x], d_projvel[x]);
	}
...
{
	//Do a quick sphere-sphere test
	float3 ab = projpos - particle1._pos;
	float lengthSq = dot(ab, ab);

	const float diameterSq = (PARTICLE_RADIUS + 0.5f) * (PARTICLE_RADIUS + 0.5f);
	if (lengthSq < diameterSq)
	{
		...
		float abnVel = dot(projvel - particle1._vel, abn);
		float jn = -(abnVel * (1.f + COLLISION_ELASTICITY));

		float overlap = PARTICLE_RADIUS + 0.5 - len;
		float b = overlap * baumgarte_factor;
...
void CudaCollidingParticles::UpdateParticles(float dt, vector < GameObject*>& proj)
{
	vector <Vector3> projveltemp(proj.size());
	vector <Vector3> projpostemp(proj.size());

	for (int i = 0; i < proj.size(); ++i)
	{
		projveltemp[i] = proj[i]->Physics()->GetLinearVelocity();
		projpostemp[i] = proj[i]->Physics()->GetPosition();
	}


	uint numElems = proj.size();
	float3* d_projvel = NULL;
	float3* d_projpos = NULL;

	if (numElems > 0)
	{
		gpuErrchk(cudaMalloc(&d_projvel, sizeof(Vector3) * numElems));
		gpuErrchk(cudaMalloc(&d_projpos, sizeof(Vector3) * numElems));

		gpuErrchk(cudaMemcpy(d_projvel, &projveltemp[0], sizeof(Vector3) * numElems, cudaMemcpyHostToDevice));
		gpuErrchk(cudaMemcpy(d_projpos, &projpostemp[0], sizeof(Vector3) * numElems, cudaMemcpyHostToDevice));
	}

	/*int j = 0;
	for (auto it1 = d_projvel->begin(); it1 != d_projvel->end(); ++it1)
	{
		for (auto it2 = d_projpos->begin(); it2 != d_projpos->end(); ++it2) {
			if (it1 == it2)
				CollideParticleWithCPUParticle(baumgarte_factor, out_p, out_p, *it2, *it1);
		}
		++j;
	}*/
	...
	for (int i = 0; i < 10; ++i)
	{
		CollideParticles<<< grid, block >>>(baumgarte_factor, num_particles, particles_ping, particles_pong, grid_cell_start, grid_cell_end, numElems, d_projpos, d_projvel);
	...
	if (numElems > 0)
	{
		gpuErrchk(cudaFree(d_projvel));
		gpuErrchk(cudaFree(d_projpos));
	}
}